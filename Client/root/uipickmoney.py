...
class PickExpandedMoneyDialog(ui.ScriptWindow):
	def __init__(self):
	...
	def LoadDialog(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, "UIScript/pickoffshopmoneydialog.py")
		except:
			import exception
	...
	def Open(self, maxValue, unitValue = 1):
		...
		if localeInfo.IsARABIC():
			self.maxValueTextLine.SetText("/" + str(localeInfo.NumberToMoneyStringWithIcon(maxValue)))
		else:
			self.maxValueTextLine.SetText(" / " + str(localeInfo.NumberToMoneyStringWithIcon(maxValue)))
	...
	def OnAccept(self):
		...
		if len(text) > 0:
			...
			moneyValue = min(value_converted, self.maxValue)
